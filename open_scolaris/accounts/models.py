...
class User(AbstractUser):
    ...
    class_object = models.ForeignKey(
        Class, on_delete=models.SET_NULL, related_name="students", null=True, blank=True
    )
    ...
    def clean(self):
        if self.role == "S" and not self.class_object:
            raise ValidationError(
                "Un élève doit impérativement appartenir à une classe"
            )

    def get_todays_courses(self):
        today = timezone.now()
        cours_list = (
            Course.objects.filter(class_object=self.class_object, date_begin__date=today)
            .order_by("date_begin")
            .all()
        )
        return today,cours_list
    
    def get_tomorrows_courses(self):
        tomorrow = timezone.now() + timedelta(days=1)
        cours_list = (
            Course.objects.filter(class_object=self.class_object, date_begin__date=tomorrow)
            .order_by("date_begin")
            .all()
        )
        return tomorrow,cours_list

    def get_n_next_due_homework(self, n: int = 3):
        homeworks = (
            Homework.objects.filter(class_object=self.class_object)
            .order_by("due_date")
            .all()[:n]
        )
        if homeworks:
            homeworks_completions = [
                HomeworkCompletion.objects.get(homework=hw, student=self)
                for hw in homeworks
            ]
    ...
    def unread_messages(self):
        messages = [
            message
            for message in OpenScolarisMessage.objects.filter(recipients=self)
            .order_by("-sent_at")
            .all()
            if not message.was_read_by(self)
        ]
        return messages
